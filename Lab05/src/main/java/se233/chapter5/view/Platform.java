package se233.chapter5.view;

import javafx.scene.image.Image;
import javafx.scene.image.ImageView;
import javafx.scene.input.KeyCode;
import javafx.scene.layout.Pane;
import se233.chapter5.Launcher;
import se233.chapter5.model.Character;
import se233.chapter5.model.Keys;

import java.util.ArrayList;

public class Platform extends Pane {
    public static final int WIDTH = 800;
    public static final int HEIGHT = 400;
    public final  static int GROUND = 300;
    private Image platformImg;
    private Character character;
    private Character character_P2;
    private ArrayList<Character> characterList;
    private ArrayList<Score> scoreList;
    private Keys keys;

    public Platform() {
        characterList = new ArrayList<>();
        scoreList=new ArrayList<>();
        keys = new Keys();
        platformImg = new Image(Launcher.class.getResourceAsStream("assets/Background.png"));
        ImageView backgroundImg = new ImageView(platformImg);
        backgroundImg.setFitHeight(HEIGHT);
        backgroundImg.setFitWidth(WIDTH);
        scoreList.add(new Score(30,GROUND + 30));
        scoreList.add(new Score(Platform.WIDTH-60,GROUND + 30));
        character = new Character(45, 45, 0, 0, KeyCode.A, KeyCode.D, KeyCode.W,"MarioSheet");
        character_P2 = new Character(755, 45, 0, 0, KeyCode.LEFT, KeyCode.RIGHT, KeyCode.UP,"megaMan");
        characterList.add(character);
        characterList.add(character_P2);
        // getChildren().addAll(backgroundImg, character,character_P2);
        getChildren().addAll(backgroundImg);
        getChildren().addAll(characterList);
        getChildren().addAll(scoreList);

        System.out.println(characterList);
      }
    public Character getCharacter() {return character;}
    public Character getCharacter_P2() {return  character_P2;}

    public Keys getKeys() {return keys;}
    public ArrayList<Character> getCharacterList() {return characterList;}

    public ArrayList<Score> getScoreList() {return scoreList;}

}
